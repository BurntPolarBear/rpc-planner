...
const gradeColor = (p) => p >= 90 ? C.green : p >= 80 ? '#0891B2' : p >= 70 ? C.gold : p >= 60 ? C.yellow : C.red;

// School year runs Aug 1 → Jul 31. Returns e.g. "2025–2026".
const schoolYearOf = (ds) => {
  const d = new Date(ds + 'T12:00:00');
  const y = d.getFullYear();
  const start = d.getMonth() >= 7 ? y : y - 1; // month 7 = August
  return `${start}\u2013${start + 1}`;
};
const CURRENT_SY = schoolYearOf(TODAY);
...
const KIND_OPTIONS = ['Assignment','Essay','Report','Project','Quiz','Test','Lab'];
...
function computeReportCard(db, studentId, schoolYear) {
...
function ReportCard({ student, db, onBack, kidView = false }) {
