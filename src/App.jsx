  ...
  const loadData = useCallback(async () => {
    try {
      const { data, error } = await supabase
        ...
        .eq('id', 1)
        .single();
      if (data?.content) setDb(data.content);
      else setDb(INIT);
    } catch {
      setDb(INIT);
    }
  ...
  return (
    <div style={{ fontFamily:'system-ui,-apple-system,sans-serif', minHeight:'100vh', background:C.bg }}>
      ...
      <header style={{ background:C.navy, height:56, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 20px' }}>
        ...
        <Btn
          onClick={() => { setMode(m => m==='student'?'parent':'student'); setView('today'); }}
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:20, flexWrap:'wrap' }}>
        {[['students','👤 Students'],['courses','📚 Courses'],['templates','📋 Templates']].map(([id,label])=>(
