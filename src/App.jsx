...
function StudentToday({ db, stuId, setStu, mut }) {
  ...
  const save = (subjectId, lessonNum, answers, date = TODAY) => mut(d => {
    const ex = d.answers.find(a => a.studentId===stuId && a.date===date && a.subjectId===subjectId);
    if (ex) { ex.answers = answers; if (ex.status !== 'needs_revision') ex.status = 'pending'; }
    else d.answers.push({ id:uid(), studentId:stuId, date, subjectId, lessonNum, answers, status:'pending', parentNote:'' });
  });

  // For question-free lessons — one tap marks it done, no review needed
  const complete = (subjectId, lessonNum, date = TODAY) => mut(d => {
    const ex = d.answers.find(a => a.studentId===stuId && a.date===date && a.subjectId===subjectId);
    if (ex) { ex.status = 'approved'; ex.parentNote = ''; }
    else d.answers.push({ id:uid(), studentId:stuId, date, subjectId, lessonNum, answers:[], status:'approved', parentNote:'' });
  });
    ...
    <div>
          ...
          {carryover.map(lesson => {
            ...
            return (
              <LessonCard
                ...
                onSave={ans => save(lesson.subjectId, lesson.lessonNum, ans, lesson.originalDate)}
                onComplete={() => complete(lesson.subjectId, lesson.lessonNum, lesson.originalDate)}
              />
            );
          })}
        </div>
        ...
        <div>
          ...
          {todayLessons.map(lesson => {
            ...
            return <LessonCard key={lesson.subjectId} subj={subj} lesson={lesson} submission={sub}
              onSave={ans => save(lesson.subjectId, lesson.lessonNum, ans)}
              onComplete={() => complete(lesson.subjectId, lesson.lessonNum)}
            />;
          })}
        </div>
      ) : null}
      ...
      {todayActivities.length > 0 && (
        <div style={{ marginTop: todayLessons.length > 0 || carryover.length > 0 ? 24 : 0 }}>
          ...
          {todayActivities.map(act => <ActivityCard key={act.id} activity={act} />)}
        </div>
      )}
    </div>
  );
...
function ActivityCard({ activity: a }) {
  return (
    <div style={{ ...card, borderLeft:`4px solid ${a.color||'#6B7280'}`, marginBottom:10, display:'flex', alignItems:'flex-start', gap:12 }}>
      <span style={{ fontSize:26, lineHeight:1, marginTop:1 }}>{a.emoji||'📌'}</span>
      <div style={{ flex:1 }}>
        <div style={{ fontWeight:700, fontSize:15, color:C.navy }}>{a.name}</div>
        {(a.time || a.location) && (
...
// ─── LESSON CARD ──────────────────────────────────────────────────────────────
function LessonCard({ subj, lesson, submission, onSave, onComplete, fromDate }) {
  const hasQuestions = (lesson.questions||[]).length > 0;
  const [open, setOpen]   = useState(
    hasQuestions && (!submission || submission.status === 'needs_revision')
  );
  ...
  const statusLabel = { approved:'✅ Approved', needs_revision:'↩ Needs revision', pending:'⏳ Awaiting review' };
  const statusColor = { approved:C.green, needs_revision:C.red, pending:C.yellow };

  const save = () => { onSave(ans); setFlash(true); setTimeout(() => setFlash(false), 2000); };

  const handleComplete = (e) => {
    e.stopPropagation();
    setCF(true);
    setTimeout(() => { onComplete(); setCF(false); }, 400);
  };

  return (
    <div style={{ ...card, borderLeft:`4px solid ${subj.color}`, marginBottom:12 }}>
      <div
        onClick={() => hasQuestions ? setOpen(o=>!o) : undefined}
        style={{ display:'flex', alignItems:'center', justifyContent:'space-between', cursor: hasQuestions ? 'pointer' : 'default', userSelect:'none' }}
      >
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          ...
          {/* Checkmark button for question-free lessons */}
          {!hasQuestions && status !== 'approved' && onComplete && (
            <button
              ...
              style={{
                ...
                background: checkFlash ? C.green : '#E8F5E9',
                color: checkFlash ? 'white' : C.green,
        ...
        <div style={{ marginTop:14, borderTop:`1px solid ${C.border}`, paddingTop:14 }}>
          ...
          {status!=='approved' && (
            <button onClick={save} style={{
              ...
              {flash ? '✓ Answers saved!' : 'Submit Answers'}
            </button>
          )}
        </div>
      )}

      {/* For question-free lessons: show instructions when expanded via notes */}
      {!hasQuestions && lesson.notes?.trim() && (
      ...
      days.forEach((day, i) => {
        ...
        if(prevLessons.length>0 && !(d.plans[planKey][day]?.length)) {
          d.plans[planKey][day]=prevLessons.map(l => {
            ...
            return { subjectId:l.subjectId, lessonNum:base+counters[l.subjectId], questions:[] };
          });
        }
      });
    });
            ...
            <DayColumn key={date} date={date} gg={gg} isToday={date===TODAY}
              ...
              onAdd={sid => mut(d => {
                ...
                if(!d.plans[planKey][date].find(l=>l.subjectId===sid)) {
                  ...
                  d.plans[planKey][date].push({subjectId:sid, lessonNum:mx>0?mx+1:startL, questions:[]});
                }
              })}
            />
          ))}
      ...
      {lessons.map(lesson => {
        ...
        return (
          ...
          >
            ...
            <div style={{ fontSize:10, color:C.muted }}>L{lesson.lessonNum} · {lesson.questions?.length||0}Q</div>
          </div>
        );
      })}
...
function LessonEditorPanel({ lesson, subj, date, templates, onSaveTemplate, onSave, onRemove, onClose }) {
  ...
  const handleSave = () => {
    onSave({ lessonNum:num, questions, notes });
      ...
      <div style={{ display:'grid', gridTemplateColumns:'180px 1fr', gap:20 }}>
        ...
        <div>
          ...
          <textarea value={qt} onChange={e=>setQt(e.target.value)}
            style={{ ...inp, width:'100%', minHeight:110, resize:'vertical', fontSize:13, display:'block', marginBottom:14 }}
    ...
    mut(d => {
      ...
      (gg?.subjects||[]).forEach(sub => {
        ...
        c.dayIdx.forEach(i => {
          const date=days[i];
          if(!d.plans[planKey][date]) d.plans[planKey][date]=[];
          if(!d.plans[planKey][date].find(l=>l.subjectId===sub.id)) d.plans[planKey][date].push({subjectId:sub.id,lessonNum:n,questions:[]});
          n++;
        });
      });
    });
  ...
  return (
    <div>
      ...
      {subs.length===0 ? (
        ...
        return <SubCard key={sub.id} sub={sub} student={student} subj={subj} questions={lesson?.questions||[]} onApprove={()=>approve(sub.id)} onRevise={note=>revise(sub.id,note)} />;
      })}
    </div>
  );
}

function SubCard({ sub, student, subj, questions, onApprove, onRevise }) {
